...
class Scraper extends ScraperAbstractInterface
{
    ...
    const EDX_DRUPAL_INDIVIDUAL = 'https://www.edx.org/api/catalog/v2/courses/%s'; // individual course page, requires course key
    ...
    */
    ...
    private function getCourseEntityFromDrupalAPI($c = array())
    {
        ...
        if ($c['max_effort'] && $c['min_effort']) {
            $course->setWorkloadMax($c['max_effort']);
            $course->setWorkloadMin($c['min_effort']);
            $course->setWorkloadType(Course::WORKLOAD_TYPE_HOURS_PER_WEEK);
        }

        if ($c['weeks_to_complete']) {
            $course->setDurationMin($c['weeks_to_complete']);
            $course->setDurationMax($c['weeks_to_complete']);
        }
        ...
        if (!empty($c['course_runs']['video']['src'])) {
            $course->setVideoIntro($c['course_runs']['video']['src']);
        }
